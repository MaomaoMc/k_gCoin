...
class GuaDan extends Component {
    ...
    componentDidUpdate(nextProps) {
        if (nextProps !== this.props) {
            ...
            if (index === -1) { // 高手挂单
                page_type = "2";
                tip = "算力≥5用户可进行10-200KJB议价交易！";
            } else {  //新手挂单
                page_type = "1";
                tip = "认证用户可进行1-10KJB议价交易！";
            }
            this.setState({
                page_type: page_type,
                tip: tip
                    ...
                    <div className="price_adjust fc_white fz_26 mb_20">
                        ...
                        <div style={{ padding: '0 .2rem' }}>
                            <div className="draft_opt" id="scroll">
                                <span className="circle" id="bar" style={{ left: parseInt(this.state.percent * 100) + "%" }}></span></div>
                        </div>
                        <input className="text_center fc_white" type="text" value={count}
                            onChange={e => {
                                this.handleIptChange({ type: "count", value: e.target.value })
                            }}

                        />
                        <p className="fc_brown fz_26">买入{count}KJB，出价{this.state.price}，总价{Math.round(parseFloat(count * this.state.price) * 100) / 100}元</p>
